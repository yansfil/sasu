...
test("blank additions fail without changing caller-owned input", () => {
  const source = [{ id: 2, title: "kept", done: false }];

  assert.throws(() => addItem(source, " \n "), /title is required/);
  assert.deepEqual(source, [{ id: 2, title: "kept", done: false }]);
});

test("summary uses the singular form only for one task", () => {
  assert.equal(summary([{ id: 1, title: "only", done: false }]), "1 item");
  assert.equal(summary([]), "0 items");
});
