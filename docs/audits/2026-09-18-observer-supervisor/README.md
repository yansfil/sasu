...
| `live-smoke.md`, `live-smoke/*` | The controlled live smoke against the installed herdr 0.9.1 with Sonnet 5 throwaway panes: S1 wake reaches the recorded pane (619 ms), S2 replaced session gets nothing, S3 wrong session refused, S4 working implementor not woken, S5 handover resumes wakes; one status-text defect found and fixed. |
| `evidence/check3-*.log`, `check3-head.txt` | The check order on the source of the observer-gone hint fix: build 0, root 97/97, unit 475/475, e2e 87/87, tsc 0. |
...
Unrun: a real launchd bootstrap of `com.sasu.supervisor` into `gui/501`; every live tick was run by hand.
