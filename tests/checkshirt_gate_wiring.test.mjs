// Skill-side wiring for the checkshirt judge gates (PRD mini-cli-llm-boundary
...
test("interview-me routes closure through the gap-audit gate", () => {
  ...
  assert.match(skill, /checkshirt gate gap-audit --slug <topic-slug> --qa-log/);
...
test("gen-prd routes readiness through the spec gate", () => {
  ...
  assert.match(skill, /checkshirt gate spec --slug <topic-slug> --prd/);
...
test("implement routes changed-code tasks through the verify gate", () => {
  ...
  assert.match(skill, /checkshirt verify --slug <topic-slug> --prd/);
...
test("every gate-calling skill keeps the override user-only in the same breath", () => {
  for (const name of ["interview-me", "gen-prd", "implement", "please"]) {
    ...
    const gateMentions = skill.match(/checkshirt (gate|verify)/g) ?? [];
    assert.ok(gateMentions.length > 0, `${name} must call a checkshirt gate`);
