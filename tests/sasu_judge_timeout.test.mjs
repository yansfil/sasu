...
// parallel refactor of the sasu judge runner must keep enforcing
...
test("sasu judge calls time out per call after the async refactor", { skip: !fs.existsSync(runnerPath) && "cli/dist not built" }, async () => {
  ...
  const emptyProject = fs.mkdtempSync(path.join(os.tmpdir(), "sasu-proj-"));
