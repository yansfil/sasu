...
- The PASS is pinned to the Decision Register's decision cells (id, kind, area, decision text, priority, status) and to the Raw Q&A answers those decisions are anchored to, and seals the review cycle: Q labels, Audit History, frontmatter, the Register's source and mapping cells, and unanchored turns (such as the one `sasu gate answer` appends for a sibling gate) may change afterwards without effect, while a changed decision, a changed anchored answer, or a newly anchored turn makes `sasu gate status` report `STALE`, and the CLI refuses automatic re-judgment until the decision is restored or the user explicitly authorizes `gate reopen`.
