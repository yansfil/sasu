...
`state.json` is the single source of truth. The views below are derived from it
and regenerated by `plan-verification`, `plan-execution`, `reconcile`,
`finalize`, and `render`; marking commands write only `state.json`, so run
`render` when a view looks stale.
