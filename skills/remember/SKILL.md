...
A good butler is never told twice.
...
| invariant | "when X changes, Y must hold" | `agents/rules/invariants/<ID>.md` via `rules add` | `deliver` pre-push gate + fulfill plan injection |
...
```sh
node ~/.codex/skills/fulfill/scripts/prd_state_harness.js rules relevant --query "<keywords>"
```
...
```sh
node ~/.codex/skills/fulfill/scripts/prd_state_harness.js rules add --file <draft.md>
```
...
```sh
node ~/.codex/skills/fulfill/scripts/prd_state_harness.js rules add --kind fact --id FACT-<name> --summary "<one line>" --evidence "<run or incident ref>" --landing docs/<page>.md
```
...
```sh
node ~/.codex/skills/fulfill/scripts/prd_state_harness.js rules add --kind regression --id REG-<name> --summary "<one line>" --evidence "<ref>" --pending
```

Pending lessons are visible debt: `deliver` warns on every ship and `doctor` reports them until they land.
...
- `deliver` matches every changed file against invariant triggers and fails closed on a failing check (`--skip-rules --reason` is the only way past, and it lands in the ship log).
- `fulfill plan-execution` injects invariants whose triggers overlap the run's write scopes as verification items, so the receipt depends on them.
- `pantry doctor` rot-checks the ledger: missing landings, dead triggers, and pending debt.
