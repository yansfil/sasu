...
test("verify runs the sealed suite, writes a current report, and starts no reviewer process", () => {
  ...
  // The CLI states facts and one move; review rules live in the skill, so the
  // assertions pin facts (tool names, the ship condition, the pointer), not prose.
  for (const text of [verified.stdout, report.agentReview.instruction]) {
    assert.match(text, /Next action: ship when the last native review covered this verified head/);
    assert.match(text, /Fidelity and Code review set/);
    assert.match(text, /Agent tool; Codex: spawn_agent; never a Herdr pane/);
    assert.match(text, /reviews-and-finalization\.md/);
    assert.match(text, /REVIEW_UNAVAILABLE/);
    assert.doesNotMatch(text, /rerun verify and review/);
  ...
  assert.match(summary, /Next: ship, after one native review set on this head unless the last review already covered it/);
});
...
test("a failed required suite writes an honest failed report and remains caller-visible", () => {
  ...
  for (const text of [failed.stdout, report.agentReview.instruction]) {
    ...
    assert.match(text, /Reproduce each in isolation/);
  ...
  assert.match(summary.stdout, /Agent Review: allowed on a committed head with verdict FAIL disclosed; delivery needs a current PASS/);
  assert.doesNotMatch(summary.stdout, /wait for a current deterministic PASS/);
  assert.match(summary.stdout, /Next: reproduce the failed required command\(s\) in isolation, fix, commit, rerun verify/);
});
...
test("a repeated FAIL on identical input is named as diagnostic, and changed input resets the count", () => {
  ...
  for (const text of [repeated.stdout, readReport(root).agentReview.instruction]) {
    ...
    assert.match(text, /Repeated input: 2 consecutive FAIL attempts/);
  }
