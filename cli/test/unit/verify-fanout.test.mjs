...
test("PRD path: an evidenced blocked verification becomes a declared gap without spending retry budget", async () => {
  const dir = makeDir();
  fs.mkdirSync(path.join(dir, "out"), { recursive: true });
  fs.writeFileSync(path.join(dir, "out", "marker.txt"), "made it");
  const prdPath = writeSemanticPrd(dir);
  const runDir = path.join(dir, "agents", "implement", "t");
  ...
  fs.writeFileSync(path.join(runDir, "state.json"), JSON.stringify({
    ...
    verification: [
      { id: "V1", status: "pending", matrix: { requiredForDone: true }, evidence: [] },
      {
        id: "V2",
        status: "blocked",
        matrix: { requiredForDone: true },
        evidence: [],
      },
    ],
    verificationPlan: {
      checks: [
        ...
        { id: "VP2", verificationId: "V2", category: "browser", requiredForDone: true, status: "planned" },
      ],
      coverage: {
        AC1: { coveredBy: ["VP1"] },
        AC2: { coveredBy: ["VP2"] },
  ...
  const beforeDeclaration = await withStub(
    dir,
    {
      verdict: "PASS",
      criteria: [
    ...
    () => runVerifyGate(dir, loadConfig(dir), "t", { prdPath, diffText: TWO_FILE_DIFF }),
  );
  assert.deepEqual(beforeDeclaration.judgedCriteriaIds, ["AC1", "AC2"], "blocked without evidence is not a declared gap");
  assert.equal(beforeDeclaration.declaredGaps, undefined);

  const statePath = path.join(runDir, "state.json");
  const implementState = JSON.parse(fs.readFileSync(statePath, "utf8"));
  implementState.updatedAt = new Date().toISOString();
  implementState.verification.find(item => item.id === "V2").evidence.push({
    text: "The external fixture required for AC2 is unavailable.",
  });
  fs.writeFileSync(statePath, JSON.stringify(implementState));

  const result = await withStub(
    dir,
    { verdict: "PASS", criteria: [{ id: "AC1", verdict: "PASS", reason: "ok", evidence: "src/widget.ts" }] },
    () => runVerifyGate(dir, loadConfig(dir), "t", { prdPath, diffText: TWO_FILE_DIFF }),
  );

  assert.equal(result.ok, false, "a declared gap keeps the whole gate closed");
  assert.equal(result.mechanical.runs[0].source, "verification-plan", "the PRD plan supplies the mechanical command");
  assert.deepEqual(result.judgedCriteriaIds, ["AC1"], "the judge sees only criteria it can still decide");
  assert.deepEqual(result.declaredGaps.map(gap => gap.criterionId), ["AC2"]);
  assert.equal(result.status.attempts, 0, "the already-declared blocker does not consume the fix budget");
  assert.equal(result.status.requiresHuman, true, "the stop flow can hand the declared gap off instead of looping");
  const record = gatesState(dir).gates.verify;
  assert.equal(record.failedStage, "declared-gap");
  assert.equal(record.totalAttempts, 2, "both the earlier PASS and uncharged handoff remain visible");
  const artifact = readArtifacts(dir).find(a => Array.isArray(a.declaredGaps));
  assert.equal(artifact.judgedVerdict, "PASS");
  assert.deepEqual(artifact.declaredGaps[0].verificationIds, ["V2"]);
});
...
test("image evidence: attachability and the byte budget are decided from the stat, once, before any read", async () => {
  ...
  // (STALE), which would mask what this test is about. AC3 is not in the PRD.
  fs.mkdirSync(path.join(dir, "out"), { recursive: true });
  fs.writeFileSync(path.join(dir, "out", "marker.txt"), "made it");
  const options = { prdPath: writeSemanticPrd(dir), diffText: SMALL_DIFF, skipMechanical: true };
...
test("evidence owned by unknown criteria is skipped; unreadable artifacts land in injectionOmitted", async () => {
  const dir = makeDir();
  fs.mkdirSync(path.join(dir, "out"), { recursive: true });
  fs.writeFileSync(path.join(dir, "out", "marker.txt"), "made it");
  const prdPath = writeSemanticPrd(dir); // AC3 is unknown to this PRD and is never judged
  const ac3Log = writeRunArtifact(dir, "agents/implement/t/artifacts/logs/ac3.log", "unknown-owner artifact body\n");
  writeImplementState(dir, {
    acceptanceCriteria: [
      { id: "AC3", artifacts: [{ kind: "log", path: ac3Log, description: "unknown AC evidence" }] },
  ...
  assert.equal(result.ok, true);
  const prompt = capturedPrompt(dir, "gate_verify-semantic");
  assert.doesNotMatch(prompt, /unknown-owner artifact body/, "an unknown criterion's artifact cannot prove a judged criterion");
  const artifact = readArtifacts(dir).find((a) => a.stage === "semantic");
